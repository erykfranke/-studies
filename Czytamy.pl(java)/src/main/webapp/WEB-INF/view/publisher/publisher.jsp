...
<div id = "container">
    <div style="background-color: #eaeaea">
        <div id = "image">
            ...
            <a href="http://localhost:8080/Czytamy_war_exploded/publisher/${publisher.id}/uploadLogo">
                <img src="${pageContext.request.contextPath}/${publisher.logo}" width="246,4‬" height="350" style="border: white 15px solid"/>
            </a>
            <%}%>
            <%}%>
            <%if(!vievfoto) {%>
        ...
        <div id = "detail">
            <div id = "title">${publisher.name}</div>
            <div style="padding-left: 40px; padding-right: 40px">
                <div class = "info"><i class="demo-icon icon-book" style="color: #FF6700"></i>${books.size()} książek</div>
                <div class = "info" style="font-size: 20px">strona www: <a href=${publisher.web_site}>${publisher.web_site}</a></div>

            </div>
        </div>
        <div style="clear: both"></div>
        <div style="padding-left: 20px; padding-right: 20px; padding-top: 10px"><hr></div>
        <div id = "description">${publisher.description}</div>
        <% if (request.getSession().getAttribute("user") != null) {
            User user1 = (User)request.getSession().getAttribute("user");
            if (user1.getRole() == 2) {%>
            <div style="float: right; margin-top: -3px; margin-right: 7px">
                <a href="http://localhost:8080/Czytamy_war_exploded/publisher/${publisher.id}/editPublisher" id="editPublisher">
                   Edytuj
                </a>
            </div>
    ...
    <div id = "listTitle">Wszysktie książki [${books.size()}]</div>
    <hr>
    <div id = "list">
