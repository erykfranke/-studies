...
<div id = "container">
    <div id = "registrationBox">
        <h1>Dodaj nowy Tag do ${book.title}</h1>
        <form:form method="POST" modelAttribute="form" action="addTag" accept-charset="UTF-8">
        <div class="element">
            <%if(request.getAttribute("error") == null) {%>
            <div>Nowy Tag: </div>
            <%} else { String error =  request.getAttribute("error").toString();%>
            <div style="float: left">Nowy Tag: </div>
            <div style="float: left; margin-left: 10px" class="error"><%=error%></div>
            <%}%>
            <form:input path="name" cssClass="textBox" placeholder = "Wpisz nowego taga"/>
        </div>
        <div style="margin-top: 80px">
            <input type="submit" placeholder="Utwórz konto" value="addTag" id = "createAccount"/>
        </div>
        </form:form>
    </div>
</div>
