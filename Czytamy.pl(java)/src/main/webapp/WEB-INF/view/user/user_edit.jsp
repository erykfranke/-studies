...
<div id = "container">
    <div id = "registrationBox">
        <h1>EDYCJA DANYCH</h1>
        <form:form method="POST" modelAttribute="user" action="edit" accept-charset="UTF-8">
            <form:hidden path="id" value="${user.id}"/>
            <div class="element">
                <div>Nick</div>
                <form:input path="nick" cssClass="textBox" placeholder = "Wpisz nick" value = "${user.nick}"/>
            ...
            <div class="element">
                <div>Miejcowość</div>
                <form:input path="place" cssClass="textBox" placeholder = "Wpisz miejscowość" value = "${user.place}"/>
            </div>
            <div class="element">
                <div>Płeć</div>
                <form:select path="gender" cssClass="select"  placeholder = "Wpisz date urodzenia">
                    <c:choose>
                        <c:when test="${user.gender.equals('Kobieta')}">
                            <form:option value="Kobieta" selected="true">Kobieta</form:option>
                            <form:option value="Mężczyzna">Mężczyzna</form:option>
                        </c:when>
                        <c:otherwise>
                            <form:option value="Kobieta" selected="true">Kobieta</form:option>
                            <form:option value="Mężczyzna" selected="true">Mężczyzna</form:option>
                        </c:otherwise>
                    </c:choose>
                </form:select>
            </div>
            <div class="element">
                <div>Twój Opis</div>
                <form:textarea path="description" cssClass="textBox" cssStyle="height: 100px" placeholder = "Napisz coś o sobie" value = "${user.description}"/>
            </div>
            <div style="margin-top: 80px">
                <input type="submit" placeholder="Utwórz konto" value="edit" id = "createAccount"/>
            </div>
        </form:form>
    </div>
</div>
